...
_wputools_complete() {
    ...
    if [ $COMP_CWORD -eq 1 ]; then
        ...
        COMPREPLY=( $(compgen -W "adminer anonymizedb archivelogs backup bduser cache cachewarm clean cleanhack codechecker dbexport dbimport debugfile detecthack diagnostic duplicatemenu generatemenus go importsite infos language login multisite-convert multisite-duplicate multisite-user muplugin nginx-convert optimizeimage plugin quickinstall sample sandbox search settings self-update src update update-quick wp wpconfig wpuwoo ${_reply}" -- $cur) )
        ...
        case "$prev" in
            ...
            ;;
            "diagnostic")
                COMPREPLY=( $(compgen -W "code-profiler cli injection memory now view web" -- $cur) )
            ;;
            "go")
