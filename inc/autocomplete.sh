...
_wputools_complete() {
    ...
    if [ $COMP_CWORD -eq 1 ]; then
        COMPREPLY=( $(compgen -W "adminer archivelogs backup bduser cache cachewarm clean cleanhack codechecker dbexport dbimport detecthack diagnostic duplicatemenu generatemenus go importsite login muplugin nginx-convert plugin sample sandbox settings self-update src update wp wpconfig wpuwoo" -- $cur) )
